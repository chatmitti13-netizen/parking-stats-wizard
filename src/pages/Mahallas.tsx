...
const Mahallas = () => {
  ...
  useEffect(() => {
    if (!isModalOpen || !mapOpen || !mapContainer.current) return;
    let map: any;
    let isMounted = true;
    ...
    const initMap = async () => {
      ...
      if (!loaded.ok || !window.mapboxgl || !window.MapboxDraw) {
        if (isMounted) {
          setMapReady(false);
          setMapLoading(false);
          setMapError(loaded.error ?? "Mapbox skriptlari yuklanmadi.");
        }
        return;
      }

      const token = import.meta.env.MAPBOX_TOKEN || import.meta.env.VITE_MAPBOX_TOKEN;
      if (!token) {
        if (isMounted) {
          setMapReady(false);
          setMapLoading(false);
          setMapError("Mapbox token topilmadi.");
        }
        return;
      }
      ...
      map.on("error", () => {
        if (isMounted) {
          setMapReady(false);
          setMapLoading(false);
          setMapError("Mapbox xaritasida xatolik yuz berdi.");
        }
      });
      map.on("load", () => {
        ...
        <CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {Array.from(new Set(mahallas.map((item) => item.assignedOfficer)))
                .filter(Boolean)
                .map((officer) => (
            ...
            <ModalContent className="max-w-5xl">
              ...
              <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
                <div className="space-y-4">
                  ...
                  <div className="grid gap-3">
                    <label className="text-sm font-medium">Mahalla logotipi</label>
                    <input
                      type="file"
                      accept="image/*"
                      className="text-sm"
                    ...
                    />
                    {formData.logo && (
                      <img src={formData.logo} alt="Mahalla logotipi" className="h-16 w-16 rounded-lg border border-border object-cover" />
                    )}
                  </div>
                  <div className="grid gap-3">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    >
                      Maydonni kartada chizish
                    </Button>
                    <Button variant="outline" onClick={handleSave}>Poligonni saqlash</Button>
                  </div>
                  {mapOpen && (
                    <div className="h-[420px] rounded-xl border border-border overflow-hidden relative">
                      {mapReady ? (
                        <div key={mapKey} ref={mapContainer} className="h-full" />
                      ) : (
                        <div className="h-full flex flex-col items-center justify-center gap-3 text-sm text-muted-foreground bg-muted/30 px-6 text-center">
                          <span>{mapLoading ? "Xarita yuklanmoqda..." : mapError ?? "Mapbox skriptlari yuklanmadi."}</span>
                          {!mapLoading && (
                            <Button
                              size="sm"
                              variant="outline"
            ...
            <TableBody>
              {paginated.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    {item.logo ? (
                      <img src={item.logo} alt={`${item.name} logo`} className="h-8 w-8 rounded-md border border-border object-cover" />
                    ) : (
